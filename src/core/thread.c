...
void nni_mtx_init(nni_mtx *mtx)
{
    nni_plat_mtx_init(mtx);
}

void nni_mtx_fini(nni_mtx *mtx)
{
    nni_plat_mtx_fini(mtx);
}

void nni_mtx_lock(nni_mtx *mtx)
{
    nni_plat_mtx_lock(mtx);
}

void nni_mtx_unlock(nni_mtx *mtx)
{
    nni_plat_mtx_unlock(mtx);
}

void nni_cv_init(nni_cv *cv, nni_mtx *mtx)
{
    nni_plat_cv_init(cv, mtx);
}

void nni_cv_fini(nni_cv *cv)
{
    nni_plat_cv_fini(cv);
}

void nni_cv_wait(nni_cv *cv)
{
    nni_plat_cv_wait(cv);
}
...
void nni_cv_wake(nni_cv *cv)
{
    nni_plat_cv_wake(cv);
}

void nni_cv_wake1(nni_cv *cv)
{
    nni_plat_cv_wake1(cv);
}

static void nni_thr_wrap(void *arg)
{
    nni_thr *thr = arg;
    int start;
...
bool nni_thr_is_self(nni_thr *thr)
{
    if(!thr->init)
    {
        return false;
...
void nni_thr_set_name(nni_thr *thr, const char *name)
{
